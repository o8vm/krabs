...
use stage_32::{load_elf, clear_bss};
use plankton::{ELF_START, IMAGE_START, INIT_SEG, KERNEL_SIZE, HEAP_START, HEAP_END};
...
fn start_32() -> ! {
    ...
    ret = unsafe { BZ2_bzBuffToBuffDecompress((ELF_START - (INIT_SEG << 4)) as *const u8,
...
extern "C" {
    fn BZ2_bzBuffToBuffDecompress(dest:       *const u8, 
                                  dest_len:   *const u32, 
                                  source:     *const u8,
                                  source_len: u32,
                                  small:      i32,
